...
const buildPayload: PayloadBuilder = async (hre, config) => {
  ...
  if (newFeeAddress === "0x0000000000000000000000000000000000000000" && !newFeeAddress.startsWith("0xPLACEHOLDER_")) {
    // Use placeholder if not set
    const placeholderAddress = "0xPLACEHOLDER_NEW_FEE_ADDRESS";
    console.warn(`   ⚠️  Using placeholder address: ${placeholderAddress} (set NEW_FEE_ADDRESS env var for actual address)`);
    return [
      {
