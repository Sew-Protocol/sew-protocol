 ...
 */
contract InsurancePoolVault is
    AccessControl,
    ReentrancyGuard
{
    ...
    function _recordDeposit(
        ...
        if (source == ISlashingModule.SlashReason.TIMEOUT_ACCEPT || 
        ...
        } else if (source == ISlashingModule.SlashReason.FRAUD ||
    ...
    function proposePayout(
        ...
        emit InsurancePayoutProposed(payoutId, to, amount, workflowId, reason, pendingPayouts[payoutId].eta);
     ...
     * @return total Total balance
     */
    function getSourceBalances() external view returns (
        uint256 timeout,
        uint256 reversal,
        uint256 fraud,
        uint256 total
     ...
     */
    function getPendingPayout(uint256 payoutId) external view returns (
        address to,
        uint256 amount,
        uint256 workflowId,
        string memory reason,
