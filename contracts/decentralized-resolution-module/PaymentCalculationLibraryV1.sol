...
contract PaymentCalculationLibraryV1 is IPaymentCalculationLibrary {
    ...
    function calculatePayments(
        ...
        for (uint256 i = 0; i < input.resolvers.length; i++) {
            ...
            payments[i] =
                (totalFees * input.resolverSharePercentage * weight) /
                (BASIS_POINTS_DENOMINATOR * totalWeight);
            addresses[i] = input.resolvers[i].resolver;
        }

        // Calculate total resolver share for validation and remainder distribution
        uint256 resolverShare = (totalFees * input.resolverSharePercentage) /
        ...
        if (paymentSum < resolverShare && payments.length > 0) {
            ...
            for (uint256 i = 0; i < payments.length && distributed < remainder; i++) {
                if (payments[i] > 0) {
                    ...
                    uint256 add = proportionalShare < remainingToDistribute
                        ? proportionalShare
     ...
     */
    function getWeightForLevel(uint8 level, Weights memory weights) private pure returns (uint256) {
