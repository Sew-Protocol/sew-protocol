...
contract MockAavePool {
    ...
    function getReserveData(address asset) external view returns (ReserveData memory) {
        ...
        return ReserveData({
            ...
            liquidityIndex: uint128(liquidityIndex[asset] > 0 ? liquidityIndex[asset] : INITIAL_LIQUIDITY_INDEX),
    ...
    function simulateYield(address token, uint256 blocks) external {
        ...
        liquidityIndex[token] = liquidityIndex[token] + (liquidityIndex[token] * YIELD_RATE * blocks) / INITIAL_LIQUIDITY_INDEX;
    }
