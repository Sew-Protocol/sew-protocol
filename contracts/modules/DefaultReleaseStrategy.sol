 ...
 * @dev Uses canonical escrowData encoding: abi.encode(token, sender, recipient, amountAfterFee, releaseAddress)
 */
contract DefaultReleaseStrategy is IReleaseStrategy, ERC165 {
     ...
     * @param escrowData Encoded as: abi.encode(token, sender, recipient, amountAfterFee, releaseAddress)
     * @return allowed True if caller is the sender (buyer) or delegated release address
     * @return reasonCode 0 if allowed, 1 if caller is not authorized
     */
    function canRelease(
        uint256 workflowId,
