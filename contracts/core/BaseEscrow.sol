...
abstract contract BaseEscrow is AccessControl, ReentrancyGuard, Pausable, SlowLaneQueueActivate {
    ...
    event EscrowStateChanged(uint256 indexed escrowId, EscrowState oldStatus, EscrowState newStatus);
    event EscrowTransferDisputed(uint256 indexed escrowId, address indexed from, address indexed to, uint256 amount);
    event EscrowTransferResolved(uint256 indexed escrowId, address indexed from, address indexed to, uint256 amount);
    event EscrowResolved(uint256 indexed escrowId, address indexed disputeResolver, bytes32 resolutionHash);
    event DisputeEscalated(uint256 indexed escrowId, uint8 fromLevel, uint8 toLevel, address indexed newDisputeResolver, address indexed escalatedBy);
    event EscalationFeeCollected(uint256 indexed escrowId, uint256 fee, address indexed feeRecipient);
    event EscrowTransferAutoReleased(uint256 indexed escrowId, address indexed to, uint256 amount);
    event EscrowTransferAutoCancelled(uint256 indexed escrowId, address indexed from, uint256 amount);
    event MaxDisputeDurationUpdated(uint256 newDuration);
    event DisputeAutoCancelled(uint256 indexed escrowId, address indexed from, uint256 amount, string reason);
    ...
    event DisputeOpened(uint256 indexed escrowId, address indexed by, address indexed disputeResolver);
    ...
    event EscrowModuleSnapshot(uint256 indexed escrowId, address resolutionModule, address releaseStrategy, address yieldGenerationModule, address yieldDistributionModule);
    event NativeETHRecovered(address indexed recipient, uint256 amount);
    event ERC20Recovered(address indexed token, address indexed recipient, uint256 amount);
    event ClaimableBalanceSet(uint256 indexed escrowId, address indexed recipient, address indexed token, uint256 amount);
    event EscrowWithdrawn(uint256 indexed escrowId, address indexed recipient, address indexed token, uint256 amount);
    ...
    function createEscrow(address token, address to, uint256 amount, EscrowSettings memory settings) public nonReentrant whenNotPaused returns (uint256) {
        ...
        escrowTransfers.push(EscrowCreationLibrary.createEscrowTransferStruct(token, to, _msgSender(), amountAfterFee, defaultResolver));
    ...
    function raiseDispute(uint256 workflowId) public returns (bool) {
        ...
        address updated = DisputeInitializationLibrary.initializeInModule(address(_getResolutionModule(workflowId)), workflowId, disputeResolver, EscrowEncodingLibrary.encodeEscrowTransferData(et.token, et.from, et.to, et.amountAfterFee));
        ...
        // TODO: Implement getIncentiveModule if needed
        // try this._getIncentiveModuleFromResolutionModule(address(resolutionModule)) returns (address incentiveModuleAddr) {
        //     if (incentiveModuleAddr != address(0)) {
        //         uint256 escrowFee = (et.amountAfterFee + (et.amountAfterFee * escrowFee / (ESCROW_FEE_DENOMINATOR - escrowFee))) - et.amountAfterFee;
        //         try IIncentiveModule(incentiveModuleAddr).onDisputeOpened(workflowId, et.token, et.amountAfterFee + escrowFee, escrowFee, 0) {}
        //         catch { /* Non-critical: Continue if incentive module call fails */ }
        //     }
        // } catch { /* Non-critical: Continue if resolution module doesn't expose incentive module */ }
    ...
    function escalateDispute(uint256 workflowId) public payable nonReentrant returns (bool success, address newDisputeResolver, uint8 newLevel) {
        ...
        DisputeOps.EscalationResult memory result = disputeOps.computeEscalation(address(resolutionModule), workflowId, _msgSender(), et.from, et.to, et.token, et.amountAfterFee, et.escrowState);
     ...
     */
    function _executeResolution(uint256 workflowId, bool isRelease, bytes32 resolutionHash) internal returns (bool) {
        _validateWorkflowId(workflowId);
        EscrowTransfer storage et = escrowTransfers[workflowId];
    ...
    function _isAuthorizedDisputeResolver(uint256 workflowId, address disputeResolver) internal view returns (bool) {
        ...
        if (snap != address(0)) {
            try IResolutionModule(snap).isAuthorizedDisputeResolver(workflowId, disputeResolver, EscrowEncodingLibrary.encodeEscrowTransferData(et.token, et.from, et.to, et.amountAfterFee)) returns (bool authorized, uint8) {
    ...
    function _getDisputeResolverForNewEscrow(uint256 workflowId, address token, address from, address to, uint256 amount) internal view virtual returns (address) {
        IResolutionModule module = _getResolutionModule(workflowId);
        if (address(module) == address(0)) revert ResolutionModuleNotConfigured();
        try module.getDisputeResolver(workflowId, EscrowEncodingLibrary.encodeEscrowTransferData(token, from, to, amount)) returns (address disputeResolver, uint8) {
    ...
    function _applyEscrowSettings(uint256 workflowId, EscrowSettings memory settings) internal {
        ...
        et.autoReleaseTime = settings.autoReleaseTime > 0 ? uint64(settings.autoReleaseTime) : (def ? uint64(timeoutConfig.defaultAutoReleaseTime) : 0);
        et.autoCancelTime = settings.autoCancelTime > 0 ? uint64(settings.autoCancelTime) : (def ? uint64(timeoutConfig.defaultAutoCancelTime) : 0);
     ...
     */
    function getTotalDeposited(uint256 workflowId) public view returns (uint256) { _validateWorkflowId(workflowId); return escrowTransfers[workflowId].amountAfterFee; }
     ...
     */
    function getEscrowCount() public view returns (uint256) { return escrowTransfers.length; }
     ...
     */
    function getEscrowStatusInfo(uint256 workflowId) public view returns (EscrowState status, bool isActive, bool isPending) {
    ...
    function _cancelAndRefund(uint256 workflowId) internal {
        EscrowTransfer storage et = escrowTransfers[workflowId]; uint256 amount = et.amountAfterFee; address from = et.from; address token = et.token;
    ...
    function _releaseEscrowTransfer(uint256 workflowId) internal {
        EscrowTransfer storage et = escrowTransfers[workflowId]; uint256 amount = et.amountAfterFee; address to = et.to; address token = et.token;
    ...
    function _emitEscrowTransferCancelled(uint256 workflowId, address token, address from, uint256 amount) internal virtual;
    function _emitEscrowTransferReleased(uint256 workflowId, address token, address to, uint256 amount) internal virtual;
    ...
    function _recordResolutionOutcome(uint256 workflowId, address disputeResolver, bool isRelease, bytes32 /* resolutionHash */) internal {
        ...
        ResolutionOutcome outcome = isRelease ? ResolutionOutcome.RELEASE : ResolutionOutcome.CANCEL;
        uint256 resolutionTime = block.timestamp;
        (bool success, ) = module.call(abi.encodeWithSignature("recordResolution(uint256,address,uint8,uint256)", workflowId, disputeResolver, uint8(outcome), resolutionTime));
        success;
    }
}
