    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        emit EvidenceModuleConfigured(maxEvidencePerDispute, allowAnyoneSubmit, allowPostResolution);
    }
    ...
    function submitEvidence(
        ...
        for (uint256 i = 0; i < currentCount; i++) {
            if (disputeEvidence[workflowId][i].hash == evidenceHash && 
        ...
        // Store evidence
        disputeEvidence[workflowId].push(EvidenceRecord({
            hash: evidenceHash,
     ...
     */
    function getEvidence(uint256 workflowId) 
        external 
        view 
        returns (
     ...
     */
    function getEvidenceRecord(uint256 workflowId, uint256 evidenceId)
        external
        view
        returns (
            bytes32 hash,
    ...
    function _canSubmitEvidenceInternal(
        ...
        if (resolutionModule != address(0)) {
            ...
            try IResolutionModule(resolutionModule).getDisputeResolver(workflowId, emptyEscrowData) 
    ...
    function supportsInterface(bytes4 interfaceId) 
        ...
        returns (bool) 
    {
        return interfaceId == type(IEvidenceModule).interfaceId ||
