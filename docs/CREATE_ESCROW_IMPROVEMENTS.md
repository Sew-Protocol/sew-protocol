    ...
    public nonReentrant whenNotPaused returns (uint256) 
{
    ...
    address defaultResolver = _getDisputeResolverForNewEscrow(
        workflowId, token, _msgSender(), to, amountAfterFee
    );
...
**Phase 2 (Medium Risk)**:
3. Remove `workflowId` from struct (verify no external reads)
4. Remove `autoReleaseTime`/`autoCancelTime` from struct (update all reads)
