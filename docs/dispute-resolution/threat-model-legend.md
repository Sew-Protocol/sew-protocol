...
- Controls: UI is _never trusted_; only on-chain escrow state + rules matter; evidence is committed on-chain (hashes) rather than "screenshots in a UI".
...
- Controls: all contracts immutable; governance can only _swap modules/templates for NEW escrows_; existing escrows remain under original rules (no rewriting history).
