...
interface IEvidenceModule is IERC165 {
  ...
  function getEvidence(
    ...
    external
    view
    returns (bytes32[] memory hashes, address[] memory submitters, uint256[] memory timestamps);
...
function submitEvidence(uint256 workflowId, string calldata evidence) external {
  ...
  if (evModule != address(0)) {
    try
      IEvidenceModule(evModule).submitEvidence(
        workflowId,
        evidenceHash,
...
event EvidenceModuleUpdated(address indexed oldModule, address indexed newModule);
```
