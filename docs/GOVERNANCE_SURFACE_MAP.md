...
| **Module Developer** | Cannot execute | Cannot execute | Cannot execute | Can upgrade (instant) | Can upgrade DecentralizedResolutionModule and ResolverIncentiveModule instantly. Cannot swap modules in BaseEscrow. |
...
| `queueDao(address)` | `ROLE_TIMELOCK` | Slow | 48h queue | Non-zero address | Queue DAO address change |
| `activateDao()` | `ROLE_TIMELOCK` | Slow | 7d + 48h | - | Activate queued DAO |
...
### Module Upgrade Lane (Instant)
- **Delay**: 0 hours (immediate)
- **Executor**: TimelockController OR Module Developer
- **Scope**: Upgrade DecentralizedResolutionModule and ResolverIncentiveModule implementations
- **Mechanism**: UUPS proxy upgrade pattern
