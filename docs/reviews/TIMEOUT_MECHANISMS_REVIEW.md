...
**Priority 2 (Should Do)**: 5. ⚠️ Create `TimeoutConfig` struct (2-3 days) - **Only if time permits** 6. ⚠️ Standardize validation bounds (1 day) - **Document difference for now**

**Priority 3 (Nice to Have)**: 7. Add automatic dispute timeout execution to `automateTimedActions()` (1 day) 8. Add timeout configuration view function (1 hour)
...
**Optional Improvements (2-3 days)**: 5. Create `TimeoutConfig` struct (if time permits) 6. Add timeout configuration view function
...
function getPendingAppealWindowDuration()
  public
  view
