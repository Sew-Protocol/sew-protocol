...
### Added
- **Frozen protocol surfaces: Appeal V1, ResolutionConfig V1, and Direct Config Selection V1** — per-escrow explicit selection binds the snapshotted resolution module, module-local config version, and immutable config root at creation. A workflow created with selectable config B continues to initialize disputes, route resolvers, quote appeals, and execute appeals under B after B is deprecated for new creation and governance activates C as the default. The legacy creation path continues to select the current active default; explicit selection rejects version zero, unknown, non-selectable, deprecated, and custom-resolver combinations before creation effects.
