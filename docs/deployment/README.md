# Deployment Docs

This folder contains deployment guides, release process notes, and network-specific runbooks.

## Start here (most used)
- **Base Sepolia core testnet**: `BASE_SEPOLIA_CORE_TESTNET_GUIDE.md`
- **Base Sepolia general deployment**: `BASE_SEPOLIA_DEPLOYMENT_GUIDE.md`
- **IEO release process**: `ieo/IEO_RELEASE_GUIDE.md`

## Releases
- `RELEASES.md`
- `RELEASE_ORGANIZATION_PROPOSAL.md`

## Dispute-resolution activation runbooks
- `dr1/DR1_ACTIVATION.md`
- `dr2/DR2_ACTIVATION.md`
- `dr3/DR3_ACTIVATION.md`

