...
### Append-only rules (normative)

Because escrows snapshot module addresses per `workflowId`, **every activated module becomes a long-lived dependency**: old escrows may continue calling old modules indefinitely.

This implies an **append-only module swap policy**:

1) **Append-only interfaces**
   - Never change an existing interface in-place (it changes behavior and/or `interfaceId`).
   - Introduce breaking changes as a new interface file (e.g., `IYieldGenerationModuleV2`).
   - Prefer ERC-165 capability detection (`supportsInterface`) over string-based version checks.

2) **Append-only module deployments**
   - Never “upgrade-in-place” a module address.
   - Deploy a new module contract for each version and swap references.
   - Do not delete or disable old modules if existing escrows may still call them.

3) **Append-only activation history**
   - Treat on-chain events (`…Queued`, `…Activated`) as the canonical append-only audit trail.
   - Optional: maintain an allowlist/registry where old modules are marked `DEPRECATED` rather than removed.

4) **Append-only compatibility guarantee**
   - A swap must never break in-flight escrows. For any module type that is snapshotted:
     - old module must remain callable and functional for escrows created under it, or
     - escrow must be designed to not depend on that module after creation (rare).

5) **Emergency / recovery**
   - If a module can take custody or approvals (e.g., yield), include a clear “down-only” emergency posture
     (pause/disable + recovery plan). Treat this as part of the module’s versioned surface.
...
## Operational “source of truth” artifacts (applies to this policy)

The append-only rules above apply to how you generate and interpret these artifacts:

- **Human address index**: `docs/deployment/deployed.md`
- **Deployment registry (append-only records, per chain)**:
  - e.g. `deploy-registry/chain-84532.json`
  - Stores deploy metadata such as constructor args, optimizer runs, timestamps, and verification state.
- **Machine snapshot / version probing**:
  - e.g. `deployments/baseSepolia/reports/version-report.json`
  - Generated by `scripts/verify-base-sepolia.ts` (ERC-165 probes + module metadata probes).
- **Hardhat deploy artifacts**:
  - `deployments/<network>/*.json` (addresses + ABIs + constructor args)
  - `deployments/<network>/solcInputs/*.json` (compiler input snapshots)
- **Git release tags**
  - Treat tags as the immutable mapping from code → deployment set (e.g. `deployed-baseSepolia-…`).

Recommendation: for each “release”, regenerate `version-report.json` and update `deploy-registry/*` + `deployed.md`,
then create an annotated git tag for that exact commit.
