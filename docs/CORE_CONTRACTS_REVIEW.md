...
function getNextWorkflowId() public view returns (uint256) { return escrowTransfers.length; }
```
...
function _requirePending(uint256 workflowId) internal view { 
    ...
    if(escrowTransfers[workflowId].escrowState != EscrowState.PENDING) revert TransferNotPending(workflowId, EscrowState.PENDING); 
}
