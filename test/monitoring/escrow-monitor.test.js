...
test('EscrowMonitor - Initialization', async (t) => {
  const config = {
  ...
  assert.equal(monitor.config.escrowVaultAddress, config.escrowVaultAddress, 'Vault address should match');
});

test('EscrowMonitor - Status Check', async (t) => {
  const config = {
    rpcUrl: 'http://localhost:8545',
    escrowVaultAddress: '0x0000000000000000000000000000000000000001',
    isDev: true,
...
test('EscrowMonitor - Event Name Detection', async (t) => {
  const config = {
    rpcUrl: 'http://localhost:8545',
    escrowVaultAddress: '0x0000000000000000000000000000000000000001',
    isDev: true,
  ...
  const pauseTopic = monitor.getEventName('0x1234567890123456789012345678901234567890123456789012345678901234');
  assert.ok(pauseTopic === null || typeof pauseTopic === 'string', 'Should return string or null');
});

test('EscrowMonitor - Pause Reason Extraction', async (t) => {
  const config = {
    rpcUrl: 'http://localhost:8545',
    escrowVaultAddress: '0x0000000000000000000000000000000000000001',
    isDev: true,
