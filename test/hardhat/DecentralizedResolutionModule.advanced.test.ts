...
describe.skip("DecentralizedResolutionModule - Advanced Unit Tests", function () {
  ...
  describe("Resolver Active Status", () => {
    ...
    it("Should reject non-timelock setting active status", async () => {
      ...
      await expect(
        module.connect(user).setResolverActive(resolver.address, false)
      ).to.be.reverted;
    });
  });
  ...
  describe("Resolution Table Category Keys", () => {
    it("Should generate unique keys for different amounts", async () => {
      const key1 = await module.generateCategoryKey(ethers.ZeroAddress, ethers.parseEther("1"), "TYPE");
      const key2 = await module.generateCategoryKey(ethers.ZeroAddress, ethers.parseEther("2"), "TYPE");
      
      expect(key1).to.not.equal(key2);
    });

    it("Should generate unique keys for different tokens", async () => {
      const key1 = await module.generateCategoryKey(resolver.address, ethers.parseEther("1"), "TYPE");
    ...
    it("Should generate unique keys for different types", async () => {
      const key1 = await module.generateCategoryKey(ethers.ZeroAddress, ethers.parseEther("1"), "TYPE1");
      const key2 = await module.generateCategoryKey(ethers.ZeroAddress, ethers.parseEther("1"), "TYPE2");
    ...
    it("Should generate same key for same inputs", async () => {
      const key1 = await module.generateCategoryKey(resolver.address, ethers.parseEther("1"), "TYPE");
      const key2 = await module.generateCategoryKey(resolver.address, ethers.parseEther("1"), "TYPE");
  ...
  describe("Round Robin Selection", () => {
    beforeEach(async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        await module.connect(seniorResolver).appointResolver(wallet.address, `R${i}`, `Resolver${i}`);
      }
    });
  ...
  describe("Escalation Config Slow Lane", () => {
    it("Should require timelock delay before activation", async () => {
      ...
      await expect(
        module.activateEscalationConfig(1)
      ).to.be.reverted;
    });
    ...
    it("Should emit events for queue and activate", async () => {
      await expect(module.queueEscalationConfig(1, {
        resolver: seniorResolver.address,
        fee: 0,
        enabled: true
      ...
      await expect(module.activateEscalationConfig(1))
        .to.emit(module, "EscalationConfigActivated");
    });
  ...
  describe("Governance Access Control", () => {
    it("Should only allow owner for certain functions", async () => {
      await expect(
        module.connect(user).appointSeniorResolver(user.address, "SR", "Senior")
      ).to.be.reverted;
    });
  ...
  describe("Edge Cases", () => {
    it("Should handle zero address resolver gracefully", async () => {
      await expect(
        module.appointSeniorResolver(ethers.ZeroAddress, "SR", "Senior")
      ).to.be.reverted;
    });

    it("Should handle empty string names", async () => {
      await expect(
        module.appointSeniorResolver(seniorResolver.address, "", "")
      ).to.not.be.reverted;
    });
    ...
    it("Should handle very long names", async () => {
      ...
      await expect(
        module.appointSeniorResolver(seniorResolver.address, longName, longName)
      ).to.not.be.reverted;
    });
