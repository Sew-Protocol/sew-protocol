...
describe("BaseEscrow Module Validation", function () {
  ...
  beforeEach(async function () {
    [deployer, timelock, resolver, standardResolver, sender, recipient, feeAddress] = await ethers.getSigners();
    ...
    defaultModule = await DefaultFactory.deploy(
      deployer.address,
      resolver.address
    );
    ...
    const tempDefaultModule = await TempDefaultFactory.deploy(
      deployer.address,
      resolver.address
    );
    ...
    await escrowableERC20.connect(timelock).queueDefaultResolutionModule(await tempDefaultModule.getAddress());
    const [, eta] = await escrowableERC20.getPendingDefaultResolutionModule();
    await time.increaseTo(Number(eta) + 1);
    await escrowableERC20.connect(timelock).activateDefaultResolutionModule();
  });

  describe("Module Interface Validation", function () {
    it("Should accept valid IResolutionModule when queueing", async function () {
      // Queue valid module (EscrowableERC20 uses queueDefaultResolutionModule)
      await expect(
        escrowableERC20.connect(timelock).queueDefaultResolutionModule(await decentralizedModule.getAddress())
    ...
    it("Should accept DefaultResolutionModule when queueing", async function () {
      await expect(
        escrowableERC20.connect(timelock).queueDefaultResolutionModule(await defaultModule.getAddress())
      ).to.emit(escrowableERC20, "DefaultResolutionModuleQueued");
    ...
    it("Should activate valid module after delay", async function () {
      // Queue module (EscrowableERC20 uses queueDefaultResolutionModule)
      await escrowableERC20.connect(timelock).queueDefaultResolutionModule(await decentralizedModule.getAddress());
      ...
      await expect(
        escrowableERC20.connect(timelock).activateDefaultResolutionModule()
  ...
  describe("Module Functionality Integration", function () {
    beforeEach(async function () {
      ...
      await decentralizedModule.connect(timelock).registerEscrowContract(await escrowableERC20.getAddress());

      // Appoint a senior resolver
      await decentralizedModule.connect(timelock).appointSeniorResolver(
        resolver.address,
      ...
      await decentralizedModule.connect(resolver).appointResolver(
      ...
      await decentralizedModule.connect(timelock).setResolutionTableEntry(defaultCategory, ResolutionTableEntry);
    });
  ...
  describe("Backward Compatibility", function () {
    it("Should work with old modules that don't have metadata functions", async function () {
      ...
      // Activate module
      await escrowableERC20.connect(timelock).queueDefaultResolutionModule(await defaultModule.getAddress());
      const [, eta] = await escrowableERC20.getPendingDefaultResolutionModule();
      await time.increaseTo(Number(eta) + 1);
      await escrowableERC20.connect(timelock).activateDefaultResolutionModule();
    ...
    it("Should handle module upgrade without breaking existing escrows", async function () {
      ...
      await escrowableERC20.connect(timelock).queueDefaultResolutionModule(await decentralizedModule.getAddress());
      [, eta] = await escrowableERC20.getPendingDefaultResolutionModule();
      await time.increaseTo(Number(eta) + 1);
      await escrowableERC20.connect(timelock).activateDefaultResolutionModule();
