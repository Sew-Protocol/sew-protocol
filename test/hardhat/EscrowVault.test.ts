...
describe('EscrowVault', function () {
  ...
  describe('Multi-Token Escrow', function () {
    it('Should create escrow for token1', async function () {
      ...
      const tx = await escrowVault
        .connect(sender)
        .createEscrow(await token1.getAddress(), recipient.address, INITIAL_TRANSFER_AMOUNT);
      await tx.wait();
    ...
    it('Should create escrow for token2', async function () {
      ...
      const tx = await escrowVault
        .connect(sender)
        .createEscrow(await token2.getAddress(), recipient.address, INITIAL_TRANSFER_AMOUNT);
      await tx.wait();
    ...
    it('Should track escrow balance per token', async function () {
      await token1
        .connect(sender)
        .approve(await escrowVault.getAddress(), INITIAL_TRANSFER_AMOUNT * 2n);
  ...
  describe('Dispute Resolution', function () {
    ...
    it('Should allow raising dispute', async function () {
      ...
      const workflowId = await createEscrowTransfer(
