before(function () {
  this.skip();
}); // Migrated to Forge: test/foundry/core/ResolverIncentiveModuleComprehensive.t.sol
/**
...
describe('ResolverIncentiveModule', function () {
  ...
  beforeEach(async function () {
    [deployer, timelock, resolver1, resolver2, resolver3, escrowContract] =
  ...
  describe('Initialization', function () {
    it('Should initialize with correct library', async function () {
      expect(await incentiveModule.currentPaymentLibrary()).to.equal(
        await paymentLibV1.getAddress(),
      );
    });
  ...
  describe('Resolver Recording', function () {
    ...
    it('Should record multiple resolvers for same dispute', async function () {
      ...
      await incentiveModule
        .connect(escrowContract)
        .recordResolver(workflowId, resolver1.address, 0);
      await incentiveModule
        .connect(escrowContract)
        .recordResolver(workflowId, resolver2.address, 1);
    ...
    it('Should reject recording from non-escrow contract', async function () {
      await expect(incentiveModule.recordResolver(1, resolver1.address, 0)).to.be.revertedWith(
  ...
  describe('Fee Recording', function () {
    it('Should record escrow fee', async function () {
      ...
      await expect(
        incentiveModule
          .connect(escrowContract)
          .recordEscrowFee(workflowId, await token.getAddress(), fee),
      )
    ...
    it('Should record escalation fees', async function () {
      ...
      await incentiveModule
        .connect(escrowContract)
        .recordEscalationFee(workflowId, await token.getAddress(), fee1);
      await incentiveModule
        .connect(escrowContract)
        .recordEscalationFee(workflowId, await token.getAddress(), fee2);
  ...
  describe('Payment Distribution', function () {
    beforeEach(async function () {
      ...
      await incentiveModule
        .connect(escrowContract)
        .recordEscrowFee(workflowId, await token.getAddress(), ethers.parseEther('100'));
      await incentiveModule
        .connect(escrowContract)
        .recordEscalationFee(workflowId, await token.getAddress(), ethers.parseEther('50'));

      // Transfer tokens to incentive module (it will distribute from its own balance)
      await token
        .connect(escrowContract)
        .transfer(await incentiveModule.getAddress(), ethers.parseEther('100'));
    });
    ...
    it('Should calculate and distribute payments', async function () {
      ...
      await expect(
        incentiveModule
          .connect(escrowContract)
          .onDisputeResolved(workflowId, await token.getAddress()),
  ...
  describe('Library Swapping', function () {
    it('Should queue new payment library', async function () {
      ...
      await expect(
        incentiveModule
          .connect(timelock)
          .queuePaymentCalculationLibrary(await paymentLibV2.getAddress()),
      )
    ...
    it('Should not activate library before delay', async function () {
      ...
      await expect(incentiveModule.connect(timelock).activatePaymentCalculationLibrary()).to.be
        .reverted;
    });
    ...
    it('Should allow rollback to previous library', async function () {
      ...
      await expect(
        incentiveModule
          .connect(timelock)
          .rollbackToPreviousLibrary(await paymentLibV1.getAddress()),
      )
  ...
  describe('Governance Functions', function () {
    it('Should queue resolver share percentage change', async function () {
      ...
      await expect(incentiveModule.connect(timelock).queueResolverSharePercentage(newPercentage))
    ...
    it('Should activate resolver share percentage after delay', async function () {
      ...
      await expect(incentiveModule.connect(timelock).activateResolverSharePercentage())
    ...
    it('Should queue weights change', async function () {
      ...
      await expect(incentiveModule.connect(timelock).queueWeights(newWeights)).to.emit(
        incentiveModule,
        'WeightsQueued',
      );
    });
    ...
    it('Should activate weights after delay', async function () {
      ...
      await expect(incentiveModule.connect(timelock).activateWeights()).to.emit(
        incentiveModule,
        'WeightsActivated',
      );
