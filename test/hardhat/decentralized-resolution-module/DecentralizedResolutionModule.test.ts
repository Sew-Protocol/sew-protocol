before(function () {
  this.skip();
}); // Migrated to Forge: test/foundry/migrated/DefaultResolutionModule.t.sol
/**
...
describe('DecentralizedResolutionModule', function () {
  ...
  beforeEach(async function () {
    ...
    await module
      .connect(timelock)
      .appointSeniorResolver(seniorResolver1.address, 'Senior Resolver 1', 'First senior resolver');
    await module
      .connect(timelock)
    ...
    await module
      .connect(seniorResolver1)
      .appointResolver(resolver1.address, 'Resolver 1', 'First resolver');
    await module
      .connect(seniorResolver1)
      .appointResolver(resolver2.address, 'Resolver 2', 'Second resolver');
    await module
      .connect(seniorResolver2)
      .appointResolver(resolver3.address, 'Resolver 3', 'Third resolver');
  });
  ...
  describe('Round-Robin Resolver Selection', function () {
    it('Should select resolvers in round-robin order', async function () {
      ...
      await module
        ...
        .initializeDispute(workflowId1, resolver1_selected, category);
      await module
        .connect(escrowContract)
        .initializeDispute(workflowId2, resolver2_selected, category);
      await module
        .connect(escrowContract)
        .initializeDispute(workflowId3, resolver3_selected, category);
    ...
    it('Should use round-robin for senior resolvers on escalation', async function () {
      ...
      await module
        .connect(escrowContract)
        .initializeDispute(workflowId, resolver1.address, category);
      ...
      expect(nextResolver1).to.be.oneOf([seniorResolver1.address, seniorResolver2.address]);
    ...
    it('Should maintain separate round-robin counters per category', async function () {
      ...
      await module
        .connect(escrowContract)
        .initializeDispute(workflowId1, resolver1_cat1, category1);
  ...
  describe('Integration with IncentiveModule', function () {
    it('Should record resolver in incentive module when dispute initialized', async function () {
      ...
      await module
        .connect(escrowContract)
        .initializeDispute(workflowId, selectedResolver, category);
  ...
  describe('Resolver Management', function () {
    it('Should allow senior resolver to appoint standard resolver', async function () {
      ...
      await expect(
        module
          .connect(seniorResolver1)
          .appointResolver(newResolverSigner.address, 'New Resolver', 'New resolver description'),
      )
    ...
    it('Should allow timelock to appoint senior resolver', async function () {
      ...
      await expect(
        module
          .connect(timelock)
          .appointSeniorResolver(newSeniorSigner.address, 'New Senior', 'New senior description'),
      )
        .to.emit(module, 'ResolverAppointed')
