...
describe("Escalation Fee Handling", function () {
  ...
  beforeEach(async () => {
    [owner, sender, recipient, resolver, seniorResolver, feeAddress, otherAccount] = await ethers.getSigners();
    ...
    await decentralizedModule.connect(owner).registerEscrowContract(await escrowableERC20.getAddress());
    
    // Appoint senior resolver
    await decentralizedModule.connect(owner).appointSeniorResolver(
    ...
    await decentralizedModule.connect(seniorResolver).appointResolver(
      resolver.address,
      "Standard Resolver",
    ...
    await decentralizedModule.connect(owner).setResolutionTableEntry(
      categoryKey,
      {
    ...
    await escrowableERC20.connect(owner).proposeResolutionModule(await decentralizedModule.getAddress());
    await time.increase(MIN_DELAY + 1);
    await escrowableERC20.connect(owner).activateResolutionModule();
    
    // Also set default resolution module (for EscrowableERC20)
    await escrowableERC20.connect(owner).queueDefaultResolutionModule(await decentralizedModule.getAddress());
  ...
  describe("Escalation Fee Collection", function () {
    ...
    it("Should emit EscalationFeeCollected event", async function () {
      await expect(escrowableERC20.connect(sender).escalateDispute(workflowId, { value: ESCALATION_FEE }))
    ...
    it("Should refund excess fee", async function () {
      ...
      const tx = await escrowableERC20.connect(sender).escalateDispute(workflowId, { value: totalSent });
    ...
    it("Should handle zero escalation fee", async function () {
      ...
      await expect(escrowableERC20.connect(sender).escalateDispute(workflowId, { value: 0 }))
    ...
    it("Should allow recipient to escalate and collect fee", async function () {
      ...
      // Recipient escalates
      await escrowableERC20.connect(recipient).escalateDispute(workflowId, { value: ESCALATION_FEE });
    ...
    it("Should revert if non-participant tries to escalate", async function () {
      await expect(
        escrowableERC20.connect(otherAccount).escalateDispute(workflowId, { value: ESCALATION_FEE })
