before(function () {
  this.skip();
}); // Migrated to Forge: test/foundry/migrated/BaseEscrow.security.test.t.sol
...
describe.skip('BaseEscrow - Edge Cases & Security Tests', function () {
  ...
  describe('Reentrancy Protection', () => {
    it('Should prevent reentrancy on release', async () => {
      ...
      await expect(escrowableERC20.connect(sender).release(workflowId)).to.not.be.reverted;
    });
    ...
    it('Should prevent reentrancy on cancel', async () => {
      ...
      await expect(escrowableERC20.connect(sender).requestCancel(workflowId)).to.not.be.reverted;
    });
    ...
    it('Should handle maximum uint256 amounts', async () => {
      ...
      await expect(
        escrowableERC20
          .connect(sender)
          .getFunction('createEscrow(address,uint256)')
          .send(recipient.address, maxAmount),
      ).to.not.be.reverted;
    });
  ...
  describe('Access Control Bypass Attempts', () => {
    it('Should prevent non-sender from releasing', async () => {
      ...
      await expect(escrowableERC20.connect(attacker).release(workflowId)).to.be.reverted;
    });
    ...
    it('Should prevent non-participant from canceling', async () => {
      ...
      await expect(escrowableERC20.connect(attacker).requestCancel(workflowId)).to.be.reverted;
    });
    ...
    it('Should prevent non-participant from raising dispute', async () => {
      ...
      await expect(escrowableERC20.connect(attacker).raiseDispute(workflowId)).to.be.reverted;
    });
    ...
    it('Should prevent unauthorized resolver actions', async () => {
      ...
      await expect(escrowableERC20.connect(attacker).resolverRelease(workflowId)).to.be.reverted;
    });
  });
  ...
  describe('State Machine Violations', () => {
    it('Should prevent releasing non-pending escrow', async () => {
      ...
      await expect(escrowableERC20.connect(sender).release(workflowId)).to.be.reverted;
    });
    ...
    it('Should prevent disputing released escrow', async () => {
      ...
      await expect(escrowableERC20.connect(sender).raiseDispute(workflowId)).to.be.reverted;
    });
    ...
    it('Should prevent canceling disputed escrow', async () => {
      ...
      await expect(escrowableERC20.connect(sender).requestCancel(workflowId)).to.be.reverted;
    });
  });

  describe('Invalid Workflow ID Handling', () => {
    it('Should reject non-existent workflow ID', async () => {
      await expect(escrowableERC20.connect(sender).release(999)).to.be.reverted;
    });

    it('Should reject workflow ID equal to nextWorkflowId', async () => {
      const nextId = await escrowableERC20.nextWorkflowId();
      await expect(escrowableERC20.connect(sender).release(nextId)).to.be.reverted;
    });

    it('Should reject workflow ID > nextWorkflowId', async () => {
      const nextId = await escrowableERC20.nextWorkflowId();
      await expect(escrowableERC20.connect(sender).release(nextId + 1n)).to.be.reverted;
    });

    it('Should handle maximum uint256 workflow ID', async () => {
      await expect(escrowableERC20.connect(sender).release(ethers.MaxUint256)).to.be.reverted;
    });
  });
  ...
  describe('Fee Calculation Edge Cases', () => {
    it('Should handle zero fee correctly', async () => {
      ...
      const zeroFeeEscrow = await ethers.deployContract('EscrowableERC20', [
        'Test',
        'TEST',
        0,
        feeAddress.address,
    ...
    it('Should handle maximum fee (100%)', async () => {
      const maxFeeEscrow = await ethers.deployContract('EscrowableERC20', [
        'Test',
        'TEST',
        10000,
        feeAddress.address,
    ...
    it('Should round down fee calculations', async () => {
      ...
      await escrowableERC20
        .connect(sender)
        .getFunction('createEscrow(address,uint256)')
        .send(recipient.address, smallAmount);
  ...
  describe('Time-based Operations', () => {
    it('Should handle auto-release timeout', async () => {
      ...
      const tx = await escrowableERC20
        .connect(sender)
        .getFunction('createEscrow(address,uint256,uint32,uint32)')
        .send(
          recipient.address,
          AMOUNT,
      ...
      await expect(escrowableERC20.automateTimedActions(workflowId)).to.not.be.reverted;
    });
    ...
    it('Should prevent execution before timeout', async () => {
      ...
      const tx = await escrowableERC20
        .connect(sender)
        .getFunction('createEscrow(address,uint256,uint32,uint32)')
        .send(recipient.address, AMOUNT, 3600, 0);
      await tx.wait();

      const workflowId = 0;

      // Try before timeout
      await expect(escrowableERC20.automateTimedActions(workflowId)).to.be.reverted;
    });
  });
    ...
    it('Should reject invalid time limits', async () => {
      ...
      await expect(
        escrowableERC20
          .connect(sender)
          .getFunction('createEscrow(address,uint256,uint32,uint32)')
          .send(recipient.address, AMOUNT, 3600, 3600),
      ).to.be.reverted;
    });
    ...
    it('Should handle zero address recipient', async () => {
      ...
      await expect(
        escrowableERC20
          .connect(sender)
          .getFunction('createEscrow(address,uint256)')
          .send(ethers.ZeroAddress, AMOUNT),
      ).to.be.reverted;
    });
  });
  ...
  describe('Partial Operations Edge Cases', () => {
    ...
    it('Should prevent partial release of zero', async () => {
      ...
      await expect(escrowableERC20.connect(sender).partialRelease(workflowId, 0)).to.be.reverted;
    });
  ...
  describe('Event Emission Validation', () => {
    ...
    it('Should emit EscrowTransferReleased', async () => {
      ...
      await expect(escrowableERC20.connect(sender).release(workflowId)).to.emit(
        escrowableERC20,
        'EscrowTransferReleased',
      );
    });
    ...
    it('Should emit DisputeOpened', async () => {
      ...
      await expect(escrowableERC20.connect(sender).raiseDispute(workflowId)).to.emit(
        escrowableERC20,
        'DisputeOpened',
      );
    });
