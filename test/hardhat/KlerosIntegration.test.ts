...
describe.skip('Kleros Integration Tests', function () {
  ...
  describe('Escrow Registration', () => {
    ...
    it('Should reject non-admin', async () => {
      await expect(klerosProxy.connect(sender).registerEscrowContract(sender.address)).to.be
        .reverted;
    });

    it('Should reject zero address', async () => {
      await expect(klerosProxy.registerEscrowContract(ethers.ZeroAddress)).to.be.revertedWith(
  ...
  describe('Dispute Creation', () => {
    ...
    it('Should create dispute', async () => {
      ...
      await expect(
        klerosProxy.createDispute(1, 2, '0x', data, { value: ARBITRATION_PRICE }),
    ...
    it('Should revert with insufficient fee', async () => {
      ...
      await expect(klerosProxy.createDispute(1, 2, '0x', data, { value: 0 })).to.be.revertedWith(
    ...
    it('Should refund excess', async () => {
      ...
      const tx = await klerosProxy.createDispute(1, 2, '0x', data, {
        value: ARBITRATION_PRICE * 2n,
      });
    ...
    it('Should reject non-escrow', async () => {
      await expect(
        klerosProxy.connect(sender).createDispute(1, 2, '0x', '0x', { value: ARBITRATION_PRICE }),
      ).to.be.reverted;
    });
  });
  ...
  describe('Evidence', () => {
    ...
    it('Should allow sender evidence', async () => {
      await expect(klerosProxy.connect(sender).submitEvidence(workflowId, 'ipfs://test')).to.emit(
        klerosProxy,
        'EvidenceSubmitted',
      );
    });
    ...
    it('Should allow recipient evidence', async () => {
      await expect(
        klerosProxy.connect(recipient).submitEvidence(workflowId, 'ipfs://test2'),
      ).to.emit(klerosProxy, 'EvidenceSubmitted');
    });

    it('Should allow anyone evidence', async () => {
      await expect(klerosProxy.connect(other).submitEvidence(workflowId, 'ipfs://test3')).to.emit(
        klerosProxy,
        'EvidenceSubmitted',
      );
    });
    ...
    it('Should reject non-existent', async () => {
      await expect(klerosProxy.submitEvidence(999, 'test')).to.be.revertedWith(
    ...
    it('Should reject after resolution', async () => {
      ...
      await expect(klerosProxy.submitEvidence(workflowId, 'late')).to.be.revertedWith(
  ...
  describe('Rulings', () => {
    ...
    it('Should reject non-arbitrator', async () => {
      await expect(klerosProxy.connect(sender).rule(0, 1)).to.be.revertedWith(
  ...
  describe('IResolutionModule', () => {
    ...
    it('Should authorize self', async () => {
      const [auth, role] = await klerosProxy.isAuthorizedDisputeResolver(
        1,
        await klerosProxy.getAddress(),
        '0x',
      );
    ...
    it('Should revert escalation', async () => {
      await expect(klerosProxy.executeEscalation(1, '0x')).to.be.revertedWith(
