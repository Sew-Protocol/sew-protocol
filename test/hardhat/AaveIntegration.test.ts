before(function () {
  this.skip();
}); // Migrated to Forge: test/foundry/migrated/AaveIntegration.test.t.sol
...
describe('Aave Integration', function () {
  ...
  beforeEach(async function () {
    [owner, sender, recipient, resolver, feeAddress, yieldRecipient1, yieldRecipient2] =
    ...
    mockPoolAddressesProvider = (await providerFactory.deploy(
      await mockAavePool.getAddress(),
    )) as MockPoolAddressesProvider;
    ...
    await aaveModule
      .connect(owner)
      .queueAavePoolProvider(await mockPoolAddressesProvider.getAddress());
    ...
    await aaveModule.registerTokenForAave(
      await escrowableERC20.getAddress(),
      await escrowTokenAToken.getAddress(),
    );
  ...
  describe('Aave Configuration', function () {
    ...
    it('Should register token for Aave', async function () {
      ...
      const isSupported = await aaveModule.isTokenSupportedByAave(
        await escrowableERC20.getAddress(),
      );
  ...
  describe('Aave Deposit Flow', function () {
    it('Should deposit to Aave when creating escrow with yield enabled', async function () {
      ...
      const aTokenBalance = await aaveModule.escrowATokenBalance(
        await escrowableERC20.getAddress(),
        workflowId,
      );
  ...
  describe('Aave Withdrawal Flow', function () {
    ...
    it('Should withdraw from Aave on release', async function () {
      ...
      await escrowableERC20
        .connect(owner)
        .transfer(await mockAavePool.getAddress(), ethers.parseEther('1000'));
  ...
  describe('Proportional Withdrawals', function () {
    ...
    it('Should handle proportional withdrawal for partial release', async function () {
      ...
      await escrowableERC20
        .connect(resolver)
        .partialReleaseAsDisputeResolver(workflowId, halfAmount, ethers.ZeroHash);
    ...
    it('Should handle proportional withdrawal for partial cancel', async function () {
      ...
      await escrowableERC20
        .connect(resolver)
        .partialCancelAsDisputeResolver(workflowId, halfAmount, ethers.ZeroHash);
  ...
  describe('Failure Scenarios', function () {
    it('Should handle Aave withdrawal failure gracefully', async function () {
      ...
      await expect(escrowableERC20.connect(sender).releaseEscrowTransfer(workflowId)).to.not.be
        .reverted;
    });
  ...
  describe('Total Deposited Tracking', function () {
    it('Should track total deposited to Aave', async function () {
      ...
      const totalAfter = await aaveModule.getTotalDepositedToAave(
        await escrowableERC20.getAddress(),
      );
