before(function () {
  this.skip();
}); // Migrated to Forge: test/foundry/migrated/EventValidation.test.t.sol
...
describe('EventValidation', () => {
  ...
  it('should emit EscrowTransferCreated when escrow is created', async () => {
    ...
    // Capture event
    await expect(tx).to.emit(vault, 'EscrowTransferCreated').withArgs(
      0n, // workflowId
      tokenAddress,
      buyer.address,
      seller.address,
  ...
  it('EscrowTransferCreated should have indexed parameters', async () => {
    ...
    const tx = await vault.connect(buyer).createEscrow(tokenAddress, seller.address, amount);
  ...
  it('should emit EscrowTransferReleased when escrow is released', async () => {
    ...
    await expect(tx)
      ...
      .withArgs(0n, tokenAddress, seller.address, amount);
  });
  ...
  it('should emit EscrowTransferCancelled when escrow is cancelled', async () => {
    ...
    await expect(tx)
      ...
      .withArgs(0n, tokenAddress, buyer.address, amount);
  });
  ...
  it('should emit EscrowStateChanged on escrow state transitions', async () => {
    ...
    // Create -> PENDING
    const createTx = vault.connect(buyer).createEscrow(tokenAddress, seller.address, amount);

    await expect(createTx).to.emit(vault, 'EscrowStateChanged');
  });
  ...
  it('event parameters should match actual escrow state', async () => {
    ...
    const createTx = await vault.connect(buyer).createEscrow(tokenAddress, seller.address, amount);
  ...
  it('should emit EscrowFeeUpdated when fee is changed', async () => {
    ...
    await expect(tx).to.emit(vault, 'EscrowFeeQueued');
  });
  ...
  it('should emit AttachmentAdded when attachment is added', async () => {
    ...
    await expect(tx).to.emit(vault, 'AttachmentAdded').withArgs(0n, uri, hash);
  });
