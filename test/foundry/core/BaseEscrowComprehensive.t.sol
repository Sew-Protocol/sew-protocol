 ...
 * @dev Goal: 100% coverage for BaseEscrow.sol
 */
contract BaseEscrowComprehensive is Test {
    ...
    function test_queueEscrowFee() public {
        ...
        (uint256 value, uint64 eta, bool exists) = vault.getPendingEscrowFee();
        assertTrue(exists);
    ...
    function test_queueEscrowFeeAddress() public {
        ...
        vm.prank(timelock);
        vault.queueEscrowFeeAddress(newFeeAddress);
    ...
    function test_automateTimedActions_single() public {
        ...
        bool success = vault.automateTimedActions(workflowId, 0);
        assertTrue(success);
    }
    ...
    function test_automateTimedActions_range() public {
        ...
        bool success = vault.automateTimedActions(workflowId1, workflowId3 + 1);
        assertTrue(success);
    }
    ...
    function test_cancelAsDisputeResolver() public {
        ...
        vm.prank(resolver);
        bool success = vault.cancelAsDisputeResolver(workflowId);
        assertTrue(success);
    }
    ...
    function test_releaseAsDisputeResolver() public {
        ...
        vm.prank(resolver);
        bool success = vault.releaseAsDisputeResolver(workflowId);
        assertTrue(success);
    }
    
    function test_partialReleaseAsDisputeResolver() public {
        uint256 amount = 1000e18;
        token.mint(buyer, amount);
        vm.prank(buyer);
        token.approve(address(vault), amount);
        ...
        uint256 remainingBalance = vault.getRemainingBalance(workflowId);
        uint256 partialAmount = remainingBalance / 2;
        
        vm.prank(resolver);
        bool success = vault.partialReleaseAsDisputeResolver(workflowId, partialAmount);
        assertTrue(success);
        
        assertEq(vault.getRemainingBalance(workflowId), remainingBalance - partialAmount);
    }
    
    function test_partialCancelAsDisputeResolver() public {
        uint256 amount = 1000e18;
        token.mint(buyer, amount);
        vm.prank(buyer);
        token.approve(address(vault), amount);
        ...
        vm.prank(resolver);
        bool success = vault.partialCancelAsDisputeResolver(workflowId, partialAmount);
        assertTrue(success);
    ...
    function test_getEscrowTransfer() public {
        ...
        // Fee is deducted, so remaining balance is less
        uint256 expectedBalance = amount - (amount * ESCROW_FEE / 10000);
        assertEq(et.remainingBalance, expectedBalance);
    }
    ...
    function test_getTotalDeposited() public {
        ...
        assertEq(vault.getTotalDeposited(workflowId), amount);
    }
    
    function test_getRemainingBalance() public {
        uint256 amount = 1000e18;
        token.mint(buyer, amount);
        vm.prank(buyer);
        token.approve(address(vault), amount);
        ...
        // Fee is deducted, so remaining balance is less
        uint256 expectedBalance = amount - (amount * ESCROW_FEE / 10000);
        assertEq(vault.getRemainingBalance(workflowId), expectedBalance);
    }
    ...
    function test_resolve() public {
        ...
        payouts[0] = Payout({recipient: seller, amount: remainingBalance});
        
        vm.prank(resolver);
        bool success = vault.resolve(workflowId, payouts, bytes32(0));
        assertTrue(success);
    }
    
    function test_resolve_partial() public {
        uint256 amount = 1000e18;
        token.mint(buyer, amount);
        vm.prank(buyer);
        token.approve(address(vault), amount);
        ...
        payouts[1] = Payout({recipient: buyer, amount: remainingBalance * 40 / 100});
