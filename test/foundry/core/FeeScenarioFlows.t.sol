...
contract MockIncentiveModule is IIncentiveModule {
    event AppealBondRecorded(uint256 workflowId, address depositor, address escalatedBy, uint256 amount, address token, uint8 newLevel);
    ...
    function getClaimablePayment(uint256, address) external pure override returns (uint256 amount) {
        return 0;
    }
    ...
    function recordAppealBond(
        ...
        emit AppealBondRecorded(workflowId, depositor, escalatedBy, amount, token, newLevel);
    }
