...
contract ResolverIncentiveModuleComprehensiveTest is Test {
    ...
    function test_RecordResolver() public {
        ...
        incentiveModule.recordResolver(1, escrow, resolver, 0);

        ResolverRecord[] memory resolvers = incentiveModule.getDisputeResolvers(1, escrow);
    ...
    function test_RecordResolver_MultipleLevels() public {
        ...
        incentiveModule.recordResolver(1, escrow, resolver2, 1);
        vm.stopPrank();
    ...
    function test_RecordResolver_RevertZeroAddress() public {
        ...
        incentiveModule.recordResolver(1, escrow, address(0), 0);
    }
    ...
    function test_RecordResolver_RevertInvalidLevel() public {
        ...
        incentiveModule.recordResolver(1, escrow, resolver, 3);
    }
    ...
    function test_RecordEscrowFee() public {
        ...
        incentiveModule.recordEscrowFee(1, escrow, address(token), fee);

        (uint256 recordedFee, ) = incentiveModule.getDisputeFees(1, escrow);
    ...
    function test_RecordEscalationFee() public {
        ...
        incentiveModule.recordEscalationFee(1, escrow, address(token), fee);

        (, uint256 recordedEscalation) = incentiveModule.getDisputeFees(1, escrow);
    ...
    function test_RecordEscalationFee_Accumulate() public {
        vm.startPrank(escrow);
        incentiveModule.recordResolver(1, escrow, resolver, 0);
        incentiveModule.recordEscalationFee(1, escrow, address(token), 50 ether);
        incentiveModule.recordEscalationFee(1, escrow, address(token), 25 ether);
        vm.stopPrank();
    ...
    function test_Escrow_PauseUnpause() public {
        ...
        incentiveModule.recordEscrowFee(1, escrow, address(token), 100 ether);
        vm.stopPrank();
        ...
        vm.prank(escrow);
        incentiveModule.recordEscrowFee(1, escrow, address(token), 100 ether);
        (uint256 fee, ) = incentiveModule.getDisputeFees(1, escrow);
    ...
    function test_ClearFeeRecording() public {
        ...
        vm.prank(timelock);
        incentiveModule.clearFeeRecording(1, escrow, "test");
    ...
    function test_onDisputeResolved_InsufficientBalance() public {
        vm.prank(escrow);
        incentiveModule.recordEscrowFee(1, escrow, address(token), 1000 ether);
        ...
        incentiveModule.onDisputeResolved(1, escrow, address(token));
    }
    ...
    function test_claimPayment_ZeroToken() public {
        ...
        incentiveModule.claimPayment(1, escrow, address(0));
    }
    ...
    function test_DistributePayments() public {
        ...
        assertTrue(incentiveModule.arePaymentsDistributed(1, escrow));

        // Claim payment (pull pattern - resolver claims their payment)
        vm.prank(resolver);
        incentiveModule.claimPayment(1, escrow, address(token));
    ...
    function test_DistributePayments_MultipleResolvers() public {
        ...
        // Setup Dispute
        vm.startPrank(escrow);
        incentiveModule.recordResolver(1, escrow, resolver, 0); // Level 0 (1x)
        incentiveModule.recordResolver(1, escrow, resolver2, 1); // Level 1 (1.5x)
        incentiveModule.recordEscrowFee(1, escrow, address(token), 1000 ether);
        vm.stopPrank();
