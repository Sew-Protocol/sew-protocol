...
contract DRv1InvariantsTest is StdInvariant, Test {
    ...
    function invariant_EMAScoreBounds() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            DecentralizedResolverStructs.ResolverStats memory stats = 
                resolutionModule.getDisputeResolverStats(resolvers[i]);
            ...
            assertTrue(
                emaScore <= EMA_PRECISION,
                "EMA score cannot exceed 1e6"
            );
        ...
        DecentralizedResolverStructs.ResolverStats memory seniorStats = 
            resolutionModule.getDisputeResolverStats(seniorResolver);
    ...
    function invariant_CounterConsistency() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            ...
            assertTrue(
                casesDecided <= casesAssigned,
                "Cannot decide more than assigned"
            );
    ...
    function invariant_TimeoutRateBounds() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            ...
            if (casesAssigned > 0) {
                ...
                assertTrue(
                    timeoutRate <= BASIS_POINTS,
                    "Timeout rate cannot exceed 100%"
                );
            }
        }
    }
    ...
    function invariant_ReversalRateBounds() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            ...
            if (casesDecided > 0) {
                ...
                assertTrue(
                    reversalRate <= BASIS_POINTS,
                    "Reversal rate cannot exceed 100%"
                );
            }
        }
    }
    ...
    function invariant_WorkloadWeightValidity() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            ...
            assertTrue(
                assignmentWeight <= BASIS_POINTS,
                "Assignment weight should be <= 10000"
            );
    ...
    function invariant_PhaseGateMetricsConsistency() public view {
        ...
        assertTrue(
            escalationRate <= BASIS_POINTS,
            "Escalation rate must be <= 100%"
        );
        ...
        assertTrue(
            activeResolvers < 1000,
            "Active resolvers must be reasonable"
        );
        
        // Average response time must be reasonable (not checking specific value,
        // but ensuring it doesn't overflow)
        assertTrue(
            avgResponseTime < type(uint128).max,
            "Avg response time should be reasonable"
        );
    }
    ...
    function invariant_LastActiveMonotonic() public view {
        for (uint256 i = 0; i < resolvers.length; i++) {
            ...
            assertTrue(
                ...
                "lastActive cannot be in future"
            );
        }
    }
}
...
contract DRv1FuzzTest is Test {
    ...
    function testFuzz_TimeoutRecording(
        uint8 numTimeouts,
        uint8 timeoutType
    ) public {
    ...
    function testFuzz_ReversalRecording(
        uint8 numDisputes,
        uint256 seed
    ) public {
        ...
        for (uint256 i = 0; i < numDisputes; i++) {
            ...
            // Resolver decides
            DecentralizedResolverStructs.ResolutionOutcome outcome1 = 
                DecentralizedResolverStructs.ResolutionOutcome.RELEASE;
            ...
            if (rand == 1) {
                ...
                DecentralizedResolverStructs.ResolutionOutcome outcome2 = 
    ...
    function testFuzz_RoundBasedDisputeFlow(
        uint8 numDisputes,
        uint256 seed
    ) public {
        ...
        for (uint256 i = 0; i < numDisputes; i++) {
            ...
            DecentralizedResolverStructs.DisputeMetadata memory dm = 
                resolutionModule.getDisputeMetadata(workflowId);
    ...
    function testFuzz_EMAAlphaParameter(
        uint256 alphaBps
    ) public {
    ...
    function testFuzz_MultipleResolversParallel(
        ...
        for (uint256 i = 0; i < numResolvers; i++) {
            DecentralizedResolverStructs.ResolverStats memory stats = 
                resolutionModule.getDisputeResolverStats(testResolvers[i]);
