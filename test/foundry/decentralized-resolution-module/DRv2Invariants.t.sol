...
contract DRv2InvariantsTest is StdInvariant, Test {
    ...
    function invariant_BondAccountingBalance() public view {
        ...
        for (uint256 workflowId = 0; workflowId < 1000; workflowId++) {
            for (uint8 round = 1; round <= 2; round++) {
                ResolverIncentiveModuleV2.AppealBondRecord memory bond = 
                    incentiveModuleV2.getAppealBond(workflowId, round);
    ...
    function invariant_EscalationHistogramAccuracy() public view {
        (uint256 round0, uint256 round1, uint256 round2) = 
            incentiveModuleV2.getEscalationDepthHistogram();
    ...
    function invariant_CostCurveMonotonic() public view {
        ...
        if (cost0 > 0) { // Only test if cost curve is enabled
...
contract DRv2FuzzTest is Test {
    ...
    function testFuzz_QuadraticCostCurve(
        ...
        if (escalationCount < 10) {
            (uint256 nextCost,) = resolutionModule.getRequiredAppealBond(0, escalationCount + 1, "");
    ...
    function testFuzz_GeometricCostCurve(
        ...
        uint256 maxExpected = baseCost * (uint256(multiplier) ** escalationCount) / (10000 ** escalationCount);
        assertTrue(actualCost <= maxExpected * 2, "Cost shouldn't exceed 2x theoretical max");
    }
    
    // ============ FUZZ TEST: Bond Distribution ============
    
    function testFuzz_BondRefund(
        uint256 workflowId,
        address depositor,
        uint128 amount
    ) public {
    ...
    function testFuzz_MultipleOperationsSequence(
