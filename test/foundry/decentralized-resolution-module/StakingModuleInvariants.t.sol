...
contract StakingModuleInvariantsTest is Test {
     ...
     */
    function testFuzz_MixConstraintsAlwaysHold(uint256 stableAmount, uint256 sewAmount) public {
        // Bound inputs
     ...
     */
    function testFuzz_CannotStakeInvalidMix(uint256 stableAmount, uint256 sewAmount) public {
    ...
    function testFuzz_ReservedCoverageNeverExceedsAvailable(
        ...
        try stakingModule.delegateStake(senior1, 0) {
            ...
            assertEq(
                reservedCoverage,
                reservedBefore + requiredCoverage,
                'Reserved increase wrong'
            );

            // INVARIANT: Available = max - reserved
            assertEq(
                availableCoverage,
                maxCoverage - reservedCoverage,
                'Available calculation wrong'
            );
            ...
            if (availableBefore >= requiredCoverage) {
                assertEq(
                    availableCoverage,
                    availableBefore - requiredCoverage,
                    'Available decrease wrong'
                );
            }
        } catch {
    ...
    function test_CoverageMultiplier() public {
        ...
        IStakingModule.DelegationInfo memory info = stakingModule.getDelegationInfo(
            resolver1,
            senior1
        );
