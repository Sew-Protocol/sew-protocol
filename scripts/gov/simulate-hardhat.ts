...
async function executeCall(
  ...
  try {
    ...
    // Execute the call
    const tx = await executorContract[call.functionName](...call.args, { value: call.value || 0 });
...
async function runPostChecks(hre: HardhatRuntimeEnvironment, calls: ProposalCall[]): Promise<void> {
...
async function main() {
  ...
  if (!proposalPath) {
    console.error(
      'Usage: pnpm ts-node scripts/gov/simulate-hardhat.ts <proposal-path> [--fork-url=<RPC_URL>] [--fork-block=<BLOCK>] [--skip-checks]',
    );
    process.exit(1);
  }
