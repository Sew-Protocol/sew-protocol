...
async function propose(
  ...
  const calldatas = await Promise.all(
    calls.map(async (c) => {
      const contract = await hre.ethers.getContractAt(c.contractName || 'Contract', c.target);
  ...
  const proposalId = await governor.hashProposal(
    ...
    values,
    calldatas,
    hre.ethers.id(proposal.description),
  );
...
async function queue(
  ...
  if (state !== 4) {
    // Succeeded
    throw new Error(
      `Proposal state is ${state}, expected 4 (Succeeded). Proposal may need voting first.`,
    );
  }
...
async function execute(
  ...
  if (state !== 5) {
    // Queued
    throw new Error(
      `Proposal state is ${state}, expected 5 (Queued). Proposal may not be ready for execution.`,
    );
  }
...
async function main() {
  ...
  if (!proposalPath) {
    console.error(
      'Usage: pnpm ts-node scripts/gov/stage.ts <proposal-path> [--stage=propose|queue|execute|all] [--network=<network>]',
    );
    process.exit(1);
  }
